...
# Configure Logfire if a token is provided. This ensures that when
# connection_checker.py is run standalone, Logfire is properly configured.
# If no token is provided, the warning is already suppressed by the env var set above.
if settings.LOGFIRE_TOKEN:
    _logfire_base_url = settings.LOGFIRE_BASE_URL
    if not _logfire_base_url and settings.LOGFIRE_TOKEN.startswith("pylf_v2_eu_"):
        _logfire_base_url = "https://logfire-eu.pydantic.dev"
    logfire.configure(token=settings.LOGFIRE_TOKEN, advanced=logfire.AdvancedOptions(base_url=_logfire_base_url) if _logfire_base_url else None)
...
def _check_neon_postgres() -> ConnectionResult:
    ...
    try:
        ...
        conn = pool.getconn(timeout=settings.POSTGRES_CONNECT_TIMEOUT)
        ...
        if pool is not None:
            try:
                pool.close(timeout=settings.POSTGRES_CONNECT_TIMEOUT)
            except Exception:
                pass
