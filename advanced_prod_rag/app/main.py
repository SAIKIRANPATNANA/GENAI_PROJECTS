        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    # Upstash/Redis can occasionally drop the connection mid-request.
                    # Failing open here prevents the rate limiter from turning into a
                    # user-facing 500 on otherwise healthy queries.
                    logfire.warning(
                        "⚠️ Rate limiter backend failed; bypassing limit for this request.",
                        storage=getattr(app.state, "rate_limiter_storage", "unknown"),
                        error=str(e),
                    )
                    return func(*args, **kwargs)
