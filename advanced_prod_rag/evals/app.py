...
with tab2:
    ...
    st.caption(
        "This eval now spaces requests out and retries on `429`, but if your backend still uses the "
        "default `RATE_LIMIT_PER_MINUTE=20`, a temporary bump for eval runs will make the pass much faster."
    )
    ...
    if run_pipeline_btn:
        ...
        mc1.metric("Correct", f"{g_metrics['correct']}/{g_metrics['scored_total']}")
        mc2.metric("Precision", f"{g_metrics['precision']:.2f}")
        mc3.metric("Recall", f"{g_metrics['recall']:.2f}")
        mc4.metric("Accuracy", f"{g_metrics['accuracy']:.2f}", delta=f"Errors {g_metrics['errors']}")
        ...
        if st.session_state.guardrails_results:
            ...
            mc1, mc2, mc3, mc4 = st.columns(4)
            mc1.metric("Correct", f"{gm['correct']}/{gm['scored_total']}")
            mc2.metric("Precision", f"{gm['precision']:.2f}")
            mc3.metric("Recall", f"{gm['recall']:.2f}")
            mc4.metric("Accuracy", f"{gm['accuracy']:.2f}", delta=f"Errors {gm['errors']}")
        ...
        if st.session_state.metric_results:
            ...
            if st.session_state.guardrails_results:
                ...
                st.metric(
                    ...
                    value=f"{gm['correct']}/{gm['scored_total']}",
                    delta=f"Precision {gm['precision']:.2f} | Recall {gm['recall']:.2f} | Errors {gm['errors']}",
                )
