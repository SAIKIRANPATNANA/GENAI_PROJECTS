...
DELAY_BETWEEN_CALLS = 20  # seconds — stays under the API's default 20 RPM rate limit
...
def _post_query(question: str, thread_id: str) -> requests.Response:
    """POST /query with retry on 429 so evals do not fail mid-run."""
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        logfire.warning(
            "⏳ Eval hit API rate limit; backing off before retry.",
            attempt=attempt,
            thread_id=thread_id,
...
def _fetch_query_result(question: str, thread_id: str) -> dict:
    ...
    resp = _post_query(question, thread_id)
    resp.raise_for_status()
    data = resp.json()
