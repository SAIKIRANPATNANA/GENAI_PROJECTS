...
For full eval runs, that default limit is usually too low because the pipeline phase and
guardrails phase together can exceed 20 `/query` calls in the same minute. For smoother
eval runs, temporarily raise `RATE_LIMIT_PER_MINUTE` to `100` in `.env`, restart FastAPI,
run the eval, then change it back if you want the stricter local setting again.
