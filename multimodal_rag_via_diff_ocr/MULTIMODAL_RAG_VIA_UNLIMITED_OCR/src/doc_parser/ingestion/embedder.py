...
class SentenceTransformerEmbedder(BaseEmbedder):
    """Free local embedder backed by sentence-transformers (no API key required).

    Runs the model on CPU by default — no GPU needed.
    On first use the model weights are downloaded from HuggingFace and cached
    in ``~/.cache/huggingface``.

    Recommended models (set via EMBEDDING_MODEL in .env):
        all-MiniLM-L6-v2          — 384 dims, ~22 MB,  very fast, good for RAG
        all-mpnet-base-v2         — 768 dims, ~420 MB, better quality
        BAAI/bge-small-en-v1.5    — 384 dims, strong semantic search
        BAAI/bge-base-en-v1.5     — 768 dims, MTEB top-tier

    Requires: ``pip install sentence-transformers>=3.0.0``
    Install:  ``uv pip install -e '.[local-embed]'``
    """
    ...
    def __init__(self, settings: "Settings") -> None:
        try:
            ...
            raise ImportError(
                ...
                "Install with: uv pip install 'doc-parser[local-embed]'"
            ) from exc
        ...
        logger.info(
            "Loading sentence-transformers model %r (first run downloads weights)",
            model_name,
        )
    ...
    def _encode_sync(self, texts: list[str]) -> list[list[float]]:
        """Run the synchronous encode call (offloaded to thread pool)."""
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        sanitised = [t if t.strip() else "[empty]" for t in texts]
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, self._encode_sync, sanitised)


_PROVIDERS: dict[str, type[BaseEmbedder]] = {
    "openai": OpenAIEmbedder,
    "gemini": GeminiEmbedder,
    "local": SentenceTransformerEmbedder,  # free, no API key, CPU-friendly
}
...
def get_embedder(settings: "Settings") -> BaseEmbedder:
    ...
    if provider == "openai" and not settings.openai_api_key:
        logger.warning(
            ...
            "Falling back to local embeddings (SentenceTransformer / BAAI/bge-small-en-v1.5)."
        )
        provider = "local"
