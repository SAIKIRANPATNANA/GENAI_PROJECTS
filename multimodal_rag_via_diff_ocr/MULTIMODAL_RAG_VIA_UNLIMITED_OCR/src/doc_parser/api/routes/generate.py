...
async def generate(req: GenerateRequest, request: Request) -> GenerateResponse:
    ...
    if groq_key_header:
        ...
        client = AsyncOpenAI(api_key=settings.groq_api_key.get_secret_value(), base_url=settings.groq_base_url)
    elif settings.openai_api_key:
        client = AsyncOpenAI(api_key=settings.openai_api_key.get_secret_value())
    else:
        raise HTTPException(
            status_code=401,
            detail="No LLM API key provided. Pass 'X-Groq-Api-Key' header in your request or set GROQ_API_KEY / OPENAI_API_KEY in your environment."
        )
    ...
    try:
        ...
        if "403" in exc_str or "forbidden" in exc_str.lower():
            logger.warning(f"Qdrant Authentication Failed: {exc}")
            raise HTTPException(
                status_code=401,
                detail="Qdrant Cloud returned 403 Forbidden. Please enter a valid Qdrant API Key in the sidebar or set QDRANT_API_KEY in your environment."
            ) from exc
    ...
    if req.rerank and candidates:
        try:
            ...
            logger.exception(f"Reranking failed: {exc}")
    ...
    model = settings.groq_text_model if (groq_key_header or settings.groq_api_key) else settings.openai_llm_model
    try:
