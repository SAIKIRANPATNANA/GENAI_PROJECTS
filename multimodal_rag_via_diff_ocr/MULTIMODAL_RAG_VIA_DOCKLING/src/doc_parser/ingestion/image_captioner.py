...
def _make_client_and_models(
    settings: "Settings",
) -> tuple[AsyncOpenAI, str, str]:
    """Return (client, vision_model, text_model) from settings.

    Priority: Groq (free tier) > OpenAI.
    - vision_model : used for image-chunk captioning (needs multimodal support)
    - text_model   : used for table / formula / algorithm captioning (text-only)
    """
    if settings.groq_api_key is not None:
        api_key = settings.groq_api_key.get_secret_value()
        client = AsyncOpenAI(api_key=api_key, base_url=settings.groq_base_url)
        vision_model = settings.groq_vision_model  # qwen/qwen3.6-27b
        text_model = settings.groq_text_model       # llama-3.3-70b-versatile
        logger.info(
            "Captioner: using Groq — vision=%s  text=%s",
        ...
        vision_model = settings.openai_llm_model   # gpt-4o
        text_model = settings.openai_llm_model     # gpt-4o
        logger.info("Captioner: using OpenAI — model=%s", vision_model)

    return client, vision_model, text_model
...
def _strip_thinking(text: str) -> str:
    ...
    Reasoning models (e.g. qwen/qwen3.6-27b) prepend an internal scratchpad
    wrapped in <think> tags before the actual answer.  We strip that block so
    downstream parsers see only the real response.

    Args:
        text: Raw LLM response string.

    Returns:
        Text with <think>...</think> blocks removed and surrounding whitespace
        stripped.
    """
    import re as _re
...
def _parse_image_response(text: str) -> tuple[str, str]:
    ...
    text = _strip_thinking(text)   # remove Qwen3 <think>...</think> blocks
    caption = ""
...
def _parse_text_response(raw_original: str, enriched: str) -> tuple[str, str]:
    ...
    enriched = _strip_thinking(enriched)   # strip Qwen3 reasoning blocks
...
async def _enrich_formula_single(
    ...
    Skips the API call when the OCR text is empty or trivially short (< 15 chars)
    — this happens when the formula was rendered as an image in the PDF and the
    OCR model returned only whitespace or the LaTeX delimiters ($$\n$$).
    """
    async with semaphore:
        try:
            raw = chunk.text.strip()

            # Guard: skip API call for empty / image-only formula OCR
            meaningful = raw.replace("$", "").replace("\n", "").strip()
            if len(meaningful) < 15:
                logger.debug(
                    "Formula chunk %s has near-empty OCR text (%r) — skipping enrichment",
                    chunk.chunk_id, raw[:40],
                )
                chunk.caption = raw if raw else "Mathematical formula (no OCR text available)"
                return
...
async def enrich_chunks(
    ...
    *,
    ...
    """Enrich all non-text chunks with LLM-generated structured descriptions.

    Dispatches by modality:
    - image     → structured TYPE / CAPTION / DETAIL / STRUCTURE description
                  (vision call with surrounding document context)
    ...
    Provider selection (in priority order):
      1. ``settings`` kwarg — auto-selects Groq or OpenAI based on which key is present
      2. ``client`` + ``model`` positional args — legacy/explicit override

    Args:
        chunks: All chunks from the document (mixed modalities).
        pdf_path: Path to the source PDF (used for image crop rendering).
        client: (Legacy) pre-built AsyncOpenAI client.  Ignored when ``settings`` is given.
        model: (Legacy) model name override.  Ignored when ``settings`` is given.
        max_concurrent: Max concurrent API calls (shared semaphore across all modalities).
        settings: Application settings — preferred over ``client``/``model``.

    Returns:
        The same list mutated in-place with enriched ``text`` and ``caption`` fields.
    """
    # ── Resolve client + models ─────────────────────────────────────────────────
    if settings is not None:
        _client, vision_model, text_model = _make_client_and_models(settings)
    elif client is not None:
        _client = client
        vision_model = model or "gpt-4o"
        text_model = model or "gpt-4o"
    else:
        raise ValueError("Provide either 'settings' or a pre-built 'client'.")
    ...
    for idx, chunk in enumerate(chunks):
        if chunk.modality == "image":
            if chunk.bbox is not None:
                ...
                tasks.append(
                    _enrich_image_single(
                        chunk, pdf_path, _client, semaphore, vision_model,
            ...
            tasks.append(
                _enrich_table_single(chunk, _client, semaphore, text_model, pdf_path=pdf_path)
            )
            counts["table"] += 1
        elif chunk.modality == "formula":
            tasks.append(_enrich_formula_single(chunk, _client, semaphore, text_model))
            counts["formula"] += 1
        elif chunk.modality == "algorithm":
            tasks.append(_enrich_algorithm_single(chunk, _client, semaphore, text_model))
            counts["algorithm"] += 1
